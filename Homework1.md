...
Translation in software engineering is to translate the problem into smaller parts and then translating those parts to even smaller parts till then can be solved individually. This type of translation is basically diving down the level of abstractions. Once the smaller level work are done we then translate it to perform higher level of work and so on till the problem is not solved. Translation not only occurs in programming but also in specification where we translate one type of specification like the use case diagram to some other type of specification like the class diagram. The work of software engineer is to translate various specifications and code so as to solve them.
